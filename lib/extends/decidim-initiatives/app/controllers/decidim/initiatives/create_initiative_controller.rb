...
module CreateInitiativeExtend
  ...
  included do
    ...
    def fill_data_step(parameters)
      enforce_permission_to :create, :initiative
      @form = build_form(Decidim::Initiatives::InitiativeForm, parameters)
      @form.attachment = form(Decidim::AttachmentForm).from_params({})
    ...
    def promotal_committee_step(parameters)
      ...
      Decidim::Initiatives::CreateInitiative.call(@form, current_user) do
        on(:ok) do |initiative|
    ...
    def initiative_type
      @initiative_type ||= Decidim::InitiativesType.find(initiative_type_id)
    end
...
Decidim::Initiatives::CreateInitiativeController.send(:include, CreateInitiativeExtend)
